...
func TestSignalChannelReceiveReturnsTypedValue(t *testing.T) {
	...
	if channel.Len() != 2 || channel.Untyped() != untyped {
		t.Fatal("typed channel did not preserve its untyped channel")
	}
}
...
func TestSignalChannelReceiveAsyncReportsAvailability(t *testing.T) {
	...
	if err != nil || ok || value != "" || !empty.asyncCall {
		t.Fatalf("ReceiveAsync() on empty channel = (%q, %t, %v), called = %t", value, ok, err, empty.asyncCall)
	}
	...
	if err != nil || !ok || value != "Temporal" {
		t.Fatalf("ReceiveAsync() on buffered channel = (%q, %t, %v)", value, ok, err)
	}
}

func TestSignalChannelReceiveAsyncReportsConsumedDecodeFailure(t *testing.T) {
	want := errors.New("decode failed")
	...
	if !errors.Is(err, want) || !ok || value != "" {
		t.Fatalf("ReceiveAsync() = (%q, %t, %v), want consumed signal and %v", value, ok, err, want)
	}
}

func TestZeroSignalChannelReportsInitializationError(t *testing.T) {
	var channel ReceiveChannel[string]
	if _, err := channel.Receive(nil); err == nil || !strings.Contains(err.Error(), "not initialized") {
		t.Fatalf("Receive() error = %v, want initialization error", err)
	}
	if _, ok, err := channel.ReceiveAsync(); ok || err == nil || !strings.Contains(err.Error(), "not initialized") {
		t.Fatalf("ReceiveAsync() = (%t, %v), want initialization error", ok, err)
	}
	if channel.Len() != 0 || channel.Untyped() != nil {
		t.Fatalf("zero channel reported %d buffered signals", channel.Len())
	}
}
